  ...
  return (
    <div className="mx-auto bg-white dark:bg-neutral-800 shadow-lg dark:shadow-cyan-400 shadow-violet-700 rounded-lg overflow-hidden grow basis-[500px]">
      <div className="p-5 flex justify-center items-center">
        {useMemo(() => {
            return content.map(({ img, zIndex, position }, index) => (
                <img className={`block w-[40px] h-[40px] rounded-full bg-white relative z-[${zIndex}] ${position} outline dark:outline-neutral-800 outline-violet-700`} src={img} key={index} />
            ))
        }, [content])}
      </div>
